...
// _______CLICK FUNCTIONS_________
...
highToLowBtn.addEventListener("click", function () {
    ...
    const sortProperties = sortPropertiesByPriceHighToLow(filteredProperties);
    populateResults(sortProperties);
})
// l-h
lowToHighBtn.addEventListener("click", function () {
    const filteredProperties = filterProperties();
    const sortProperties = sortPropertiesByPriceLowToHigh(filteredProperties);
    populateResults(sortProperties);
})
// a-z
alphabeticalBtn.addEventListener('click', function () {
    const filteredProperties = filterProperties();
    const sortedProperties = sortPropertiesAlphabetical(filteredProperties); // sorting filtered properties by price low to high
...
function filterProperties() {
    const filteredProperties = properties.filter(property => {
        ...
        // that it doesn't match so it reuters false and doesn't add to the array
        if (filterValue && !propertyLocation.includes(filterValue)) {
            // EXAMPLE if (Auckland && !propertyLocation.includes(Auckland))
            return false
        }
    ...
    // return the filtered propertied array
...
// Sort Results - sort by price highest to lowest
...
function attachModalToImages() {
    ...
    // run for loop over the imgs to add a click to each imgs
...
const swiper = new Swiper('.swiper', {
    ...
    pagination: {
        el: '.swiper-pagination',
        clickable: true, // makes it clickable
    },
});